...
function model_thread_result_get($urls_array,$loadPath)   
  {  
      ...
      {  
        ...
        $thread_array[$thread_array_key]->lock();
      }
      ...
      if($urldetail){
        foreach ($urldetail as $key => $value) {
          ...
          /*单页面循环*/
          // $detail_data=model_http_curl_get($detail_url);
          // preg_match_all("#<img.*?class=\"zoom\" file=\"(.*?)\"#",$detail_data,$imgSrc);
          // if($imgSrc[1]){
          ...
          // }
          /*多线程*/
          $detail_array[$key] = new catchImg($detail_url);
          $detail_array[$key]->start();
        ...
        foreach($detail_array as $detail_array_key=>$detail_array_value){
          ...
          if($detail_array[$detail_array_key]->join()){
            preg_match_all("#<img.*?class=\"zoom\" file=\"(.*?)\"#",$detail_array[$thread_array_key]->data,$imgSrc);
            if($imgSrc[1]){
              foreach($imgSrc[1] as $imgSrcValue){
